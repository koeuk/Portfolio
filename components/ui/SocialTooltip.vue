  ...
  <ul :class="['flex items-center justify-center gap-4', $attrs.class]">
    <li v-for="(item, index) in items" :key="index" class="relative group">
      <a :href="item.href" :aria-label="item.ariaLabel" target="_blank" rel="noopener noreferrer"
        class="relative flex items-center justify-center w-12 h-12 rounded-full bg-white overflow-hidden transition-all duration-300 ease-in-out">
        <!-- Color fill that rises on hover -->
        <span class="absolute bottom-0 left-0 w-full h-0 transition-all duration-300 ease-in-out"
          :style="{ backgroundColor: item.color }" />
        <!-- Icon (uses currentColor so it flips to white on hover) -->
        <svg viewBox="0 0 24 24" fill="currentColor"
          class="relative z-10 w-6 h-6 text-gray-800 transition-colors duration-300 ease-in-out">
          <path :d="item.path" />
        </svg>
      </a>
      <!-- Tooltip -->
      <span :style="{ backgroundColor: item.color }"
        class="absolute left-1/2 -translate-x-1/2 bottom-[-40px] px-2.5 py-1.5 text-sm text-white whitespace-nowrap rounded-md opacity-0 invisible transition-all duration-300 ease-in-out">
        {{ item.tooltip }}
      </span>
    </li>
  </ul>
