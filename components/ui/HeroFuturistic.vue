      ...
      <div class="hero-copy">
        ...
        <nav class="hero-socials" aria-label="Social links">
          ...
          <button
            type="button"
            class="cv-button"
            :style="{ animationDelay: `${0.78 + socials.length * 0.06}s` }"
            @click="cvOpen = true"
          >
            <svg class="cv-button-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
...
/* "View my CV" pill sits at the end of the social row */
.cv-button {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
  ...
  color: var(--gold);
  background: rgba(211, 176, 120, 0.08);
  font-size: 0.78rem;
  font-weight: 600;
  letter-spacing: 0.04em;
  white-space: nowrap;
  cursor: pointer;
  transition: transform 0.35s cubic-bezier(0.22, 1, 0.36, 1), color 0.3s ease, background 0.3s ease, box-shadow 0.3s ease;
  animation: hero-rise 0.75s cubic-bezier(0.22, 1, 0.36, 1) both;
}
...
.cv-button:hover {
  ...
  color: #17130c;
  background: var(--gold);
  box-shadow: 0 10px 24px rgba(211, 176, 120, 0.25);
}

.cv-button-icon { width: 15px; height: 15px; }
